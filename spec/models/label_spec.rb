...
RSpec.describe Label, type: :model do
  ...
  let(:user) { build(:user) }
  let(:post) { build(:post) }
  let(:label) { FactoryGirl.create(:label, name: 'label1') }
  let(:label2) { FactoryGirl.create(:label, name: 'label2') }
  ...
  describe "labelings" do
    it "allows the same label to be associated with a differet topic and post" do
      topic.labels << label
      post.labels << label
